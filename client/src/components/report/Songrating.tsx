...
const SongRatingReport: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch the song ratings from backend API
        ...
        if (error.response && error.response.status === 401) {
          ...
          setErrMsg('Failed to fetch song rating report.');
        }
      }
    };

    // Fetch song ratings when component mounts
    fetchData();
  }, []);

  const columns = React.useMemo<ColumnDef<SongReportData>[]>(() => [
    {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="bg-[#121212] text-[#EBE7CD] p-8 flex-grow font-sans">
        <h1 className="text-3xl font-semibold mb-8">Song Rating Report</h1>
